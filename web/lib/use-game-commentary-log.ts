 ...
 * calling component stays mounted. Shared by `GameFeed.tsx` (single-game
 * standalone view) and the homepage board's feed ticket (fed by the
 * board's own already-running stream via a passed-in log instead -- see
 * `recent-games-board.tsx`) so both accumulate history the same way:
 * unbounded array, deduped only against the immediately-previous entry.
 * Deliberately ephemeral (no persisted table), matching this feature's
 * non-goals.
 *
