...
- The LLM integration is provider-agnostic: `search-loop.ts` depends only on a shared `LlmClient` interface, never a specific SDK, so a new provider is addable without touching the loop itself. The active provider is selected via `SEARCH_LLM_PROVIDER`, defaulting to **Groq** (`openai/gpt-oss-20b` via `groq-sdk`) — switched from an initial Gemini 3.8 Flash default after live production testing found repeated real `503 UNAVAILABLE` "high demand" errors on that just-launched free-tier model; Groq runs on different infrastructure (its own LPU hardware, not GPU-based) and was live-verified end-to-end before becoming the default. Gemini (`gemini-3.8-flash`) and Anthropic (Claude Haiku 4.5) remain fully supported as explicit alternatives. A missing/invalid API key for the selected provider fails fast with a clear config error, never a silent fallback to another provider.
- A provider/config failure (e.g. a missing API key, or the upstream LLM API itself erroring) renders as a distinct "search is temporarily unavailable" state — a separate SSE `event: error`, never conflated with CAP-5's honest "no data" state.
